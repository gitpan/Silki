...
Silki::Manual::Admin - Hack on Silki
...
=item * F<dev-bin/initialize-db>

This rebuild the Silki schema and populates it with some initial users and
wikis. Run it with the C<--help> flag to see options. If you want to drop the
existing schema you'll have to pass the C<--drop> flag.
...
Storable to store a copy of the schema in a file. You can regenerate this copy
